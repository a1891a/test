...
public class AnalysisController {
    ...
    public Result getAnalysisList(@RequestBody ReqData<Analysis> reqData,HttpServletRequest request) {
        ...
        switch (user.getRole().getValue()) {
            //学生
            case 1:
                return new Result(getStuAnalysisData(reqData,user), "请求成功");
            //教师
            //管理员
            //超级管理员
            case 2:
            case 3:
            case 4:
                return new Result(getAnalysisData(reqData), "请求成功");
        }
        return new Result("0", "请求失败");
    }

    //学生查看的
    private Map getStuAnalysisData(ReqData<Analysis> reqData,User user) {
    ...
    //教师查看的
    private Map getAnalysisData(ReqData<Analysis> reqData) {
        List<Analysis> analysisList = EntityUtils.getListByPage(analysisService, reqData);
        int analysisCount = EntityUtils.getListRecordByQuery(analysisService, reqData);
        List<User> userList = getUserList(analysisList);
        List<Project> projectList = getProjectList();
        List<Module> modList = getModuleList();
        Map<String, Object> data = getData(analysisList, analysisCount, userList, projectList, modList);
    ...
    @RequestMapping("/commentsAnalysis")
    public Result commentsAnalysis(@RequestBody ReqData<Analysis> reqData) {
        UpdateWrapper updateWrapper = EntityUtils.getCommentsWrapper(reqData);
        updateWrapper.set("update_date", DateUtils.getCurrentData("yyMMdd"));
        updateWrapper.eq("tc_code", reqData.getTcCode());
        boolean update = analysisService.update(null, updateWrapper);
        if (update) {
            //更新删除后的列表给前端
            return new Result(getAnalysisData(reqData), "修改点评成功");
        }
        return new Result("0", "修改点评失败");
    }

    @RequestMapping("/approvalAnalysis")
    public Result approvalAnalysis(@RequestBody ReqData<Analysis> reqData) {
        UpdateWrapper updateWrapper = EntityUtils.getApprovalWrapper(reqData, "tc_code");
        updateWrapper.set("update_date", DateUtils.getCurrentData(DateString.yyMMdd));
        boolean update = analysisService.update(null, updateWrapper);
        if (update) {
            return new Result(getAnalysisData(reqData), "审核成功");
        }
